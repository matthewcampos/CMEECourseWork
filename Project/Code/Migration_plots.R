...
#Titles for legend
condition <- c('No Mig.', 'Random Mig.', 'Mig. 1% q1Gen.', 'Mig. 3% q1Gen.', 'Mig. 5% q1Gen.','Mig. 1% q5Gen.',
...
plot.colour <- rainbow(44, start = 0, end = 0.30) #colour for each line
...
for (l in 1:4){
  ...
  for (k in 1:44){
    ...
    for (j in 1:15){
      ...
      fit.list.k[[length(fit.list.k)+1]] <- fit #adds 30 fit runs per conditions to each list
    }
    array <- do.call(cbind, heterogenous.fit.list) #converts to an array
    ...
    if (k == 1){
      ...
      lines(1:length(mean.array),as.numeric(mean.array),type='l',lty=k,col=plot.colour[k]) #adds other 43 runs 
    }
    legend("bottomright", legend = legend.title,col=plot.colour, lty=1:44, cex=0.8)
    dev.off()
  }
}
#List to save 44 fitnesses
