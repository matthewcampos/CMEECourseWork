...
subfolder_name <- paste0("run_",c(1:10))
mainDir <- '../gns_50-80'
...
for (run in 1:1){
  ...
  for (loop in 1:10){
    ...
    for (i in 1:generations){
      ...
      migrant_y_1 <- (migrant_mu$`11` * negative_R_j(migrant_y_j$`2`,migrant_theta$`21`,migrant_P$`21`)) + (migrant_mu$`12` * negative_R_j(migrant_y_j$`1`,migrant_theta$`22`,migrant_P$`22`))
      ...
      #add to pop.list
      pop.list[[heading]] <- new_gen
      migrantpop.list[[heading]] <- migrant_new_gen
    ...
    save(fit,avg_fit,condition,pop.list,migrantpop.list,chance.record,file = filename)
    
    file <- paste0(path[run],'/','Traits/','Simulation',loop,".rda", sep="")
    save(w_bar_list,avg_trait,condition,max_trait,yvalues.list,mig_yvalues.list,file = file)
  }
