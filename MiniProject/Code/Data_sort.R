...
GrowthData <- read.csv("../Data/Growth_data.csv", header = TRUE)

#Getting unique ID's by subsetting by Species, Medium, Temp then Citation
pdf(paste("../Results/SpeciesID.pdf")) #open pdf
species <- unique(GrowthData[ ,"Species"]) #find unique species
for (s in species){ 
  species_data <- filter(GrowthData, Species==s) #uses dplyr to group by species
    ...
    for (t in temperature){
      ...
      for (c in citation){
        ...
        for (r in rep){
          ...
          filename <- paste("../Data/RDA/",m,"_",s,"_",t,"_",r,".rda",sep = "")
          save(ID,data,file = filename)
        } 
